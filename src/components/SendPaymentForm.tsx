...
import { Send, Copy, QrCode, Check, ArrowLeft } from "lucide-react";
      ...
      >
        <div className="border-b border-border px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display text-xl text-foreground">Send Payment</h2>
          </div>
        </div>
        ...
        <div className="p-6">
          <AnimatePresence mode="wait">
              ...
              <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    className="w-full rounded-xl border border-border bg-background py-4 pl-10 pr-4 text-3xl font-bold text-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:ring-2 focus:ring-ring"
                  />
                </div>
                <input
                  value={recipient}
                  onChange={(e) => setRecipient(e.target.value)}
                  placeholder="Email or phone (optional — generates link)"
            ...
            {step === "done" && (
              <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-5 text-center">
                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-primary/10">
                  <Check className="h-7 w-7 text-primary" />
                </div>
                <h3 className="font-display text-xl text-foreground">Payment Created!</h3>
                ...
                <div className="flex items-center gap-2 rounded-xl border border-border bg-secondary/50 p-3">
                  <span className="flex-1 truncate text-sm text-foreground">{generatedLink}</span>
                  <button onClick={copyLink} className="rounded-lg border border-border bg-card p-2 transition-colors hover:bg-secondary">
                    {linkCopied ? <Check className="h-4 w-4 text-primary" /> : <Copy className="h-4 w-4 text-muted-foreground" />}
                  </button>
                  <button className="rounded-lg border border-border bg-card p-2 transition-colors hover:bg-secondary">
